...
class tax_report_invoices(report_sxw.rml_parse, common_report_header):
    ...
    def _get_codes(self, based_on, company_id, parent=False, level=0, period_list=[], context=None):
        obj_tc = self.pool.get('account.tax')
        ids = obj_tc.search(self.cr, self.uid, [('company_id','=',company_id)], order='sequence', context=context)
        lst_tax_code = []
        for obj in obj_tc.browse(self.cr, self.uid, ids):
            lst_tax_code += [obj.base_code_id]
        lst_tax_code = list(set(lst_tax_code))
        res = []
        for code in lst_tax_code:#obj_tc.browse(self.cr, self.uid, ids, {'based_on': based_on}):
    ...
    def _get_account_move_lines(self, tax_id, based_on, company_id=False, context=None):
        ...
        fields = [
                     ...
                     'act.code',
                     'act.name',
                     'inv.origin as reference',
                     'part.name as partner',
                 ]

        period_lst = str(self.an_period_ids).replace('[','(').replace(']',')')
        ...
        sql += 'GROUP BY aml.move_id,inv.number,aml.date,inv.date_invoice,act.code,act.name,inv.origin,part.name '
